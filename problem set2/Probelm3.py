# -*- coding: utf-8 -*-
"""
Created on Sun May 13 02:20:14 2018
...
MinMonthPay = 0
lower = balance / 12
upper = (balance*(1+MonIntRate)**12)/12




while round(UpdatedBalanceEachMonth,2) != 0:
    MinMonthPay = (upper+lower)/2
    UpdatedBalanceEachMonth = balance
    for i in range(12):
        UpdatedBalanceEachMonth = (UpdatedBalanceEachMonth-MinMonthPay)+(UpdatedBalanceEachMonth-MinMonthPay)*MonIntRate
    if UpdatedBalanceEachMonth > 0 :
        lower = (upper+lower)/2
    if UpdatedBalanceEachMonth < 0 :
        upper = (upper+lower)/2
    
print("Lowest Payment: ",round(MinMonthPay,2))